...
void MyMapWidget::init()
{
	...
	*/
	...
	_font.InitFont(wglGetCurrentDC(), (wchar_t*)std::wstring(L"Arial").c_str(), 22);
	MeteLayer::_pCB = this;
}
...
void MyMapWidget::viewShowBackground(bool on){
	_displayStates._bShowBackground = on; update();
};

void MyMapWidget::onCheckShowBeliefEllipse(bool bChecked) {
	_displayStates._bShowBeliefEllipse = bChecked; update();
}
...
void MyMapWidget::viewShowContourLine(bool on){
	_displayStates._bShowContourLine = on; update(); };

void MyMapWidget::viewShowContourLineSorted(bool on) {
	_displayStates._bShowContourLineSorted = on; update();
};

void MyMapWidget::viewShowContourLineSortedSDF(bool on) {
	_displayStates._bShowContourLineSortedSDF = on; update();
};

void MyMapWidget::viewShowContourLineResampled(bool on) {
	_displayStates._bShowContourLineResampled = on; update();
};

void MyMapWidget::viewShowContourLineDomainResampled(bool on) {
	_displayStates._bShowContourLineDomainResampled = on; update();
};

void MyMapWidget::viewShowContourLineSDF(bool on) {
	_displayStates._bShowContourLineSDF = on; update();
};

void MyMapWidget::viewShowContourLineMin(bool on){
	_displayStates._bShowContourLineMin = on; update();
};

void MyMapWidget::viewShowContourLineMax(bool on){
	_displayStates._bShowContourLineMax = on; update();
};

void MyMapWidget::viewShowContourLineMean(bool on){ 
	_displayStates._bShowContourLineMean = on; update();
};

void MyMapWidget::viewShowContourLineMedian(bool on) {
	_displayStates._bShowContourLineMedian = on; update();
};

void MyMapWidget::viewShowContourLineOutlier(bool on) {
	_displayStates._bShowContourLineOutlier = on; update();
};

void MyMapWidget::viewShowCluster(bool on) {
	_displayStates._bShowCluster = on; update();
};

void MyMapWidget::viewShowRCBands(bool on) {
	_displayStates._bShowRCBands = on; update();
};
...
void MyMapWidget::onIsoValue0(bool bState) { _pEnsLayer->_arrIsoValueState[0] = bState; update(); };
void MyMapWidget::onIsoValue1(bool bState) { _pEnsLayer->_arrIsoValueState[1] = bState; update(); };
void MyMapWidget::onIsoValue2(bool bState) { _pEnsLayer->_arrIsoValueState[2] = bState; update(); };
void MyMapWidget::onIsoValue3(bool bState) { _pEnsLayer->_arrIsoValueState[3] = bState; update(); };
void MyMapWidget::onIsoValue4(bool bState) { _pEnsLayer->_arrIsoValueState[4] = bState; update(); };
void MyMapWidget::onIsoValue5(bool bState) { _pEnsLayer->_arrIsoValueState[5] = bState; update(); };
void MyMapWidget::onIsoValue6(bool bState) { _pEnsLayer->_arrIsoValueState[6] = bState; update(); };
void MyMapWidget::onIsoValue7(bool bState) { _pEnsLayer->_arrIsoValueState[7] = bState; update(); };
void MyMapWidget::onIsoValue8(bool bState) { _pEnsLayer->_arrIsoValueState[8] = bState; update(); };
void MyMapWidget::onIsoValue9(bool bState) { _pEnsLayer->_arrIsoValueState[9] = bState; update(); };
